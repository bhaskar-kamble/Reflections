#animation of wave reflection at a boundary as superposition of two waves - open BC
...
line5=ax.plot([],[],linestyle="none",marker="o",markersize=4,color="black")[0]
lines.append(line5)
...
tmin = -20.0
tmax = 30.0
time = np.linspace(tmin,tmax,500)
lmbda = 3.0
bias = 0.2
...
def animate(i):
    for lnum,line in enumerate(lines):
        ...
        if lnum==4:
            line.set_data([0,0],[y3[x.size/2],y3[x.size/2]])
    return lines    
nzt = time.size-1
anim = animation.FuncAnimation(fig, animate, init_func=init,frames=nzt, interval=5, blit=True)
#anim.save("rope_05_openBC_superposition.gif",dpi=80,writer='imagemagick')
plt.show()
